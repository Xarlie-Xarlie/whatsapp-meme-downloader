...
RUN echo 'Package: *\\ Pin: origin packages.mozilla.org Pin-Priority: 1000' | tee /etc/apt/preferences.d/mozilla RUN apt-get clean

RUN apt-get update && apt-get install firefox

WORKDIR /app

RUN rm -rf ~/.config/google-chrome-stable
